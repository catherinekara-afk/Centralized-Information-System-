...
function apiFetch(url, options = {}) {
    ...
    const token = adminData.authToken || sessionStorage.getItem('auth_token');
    if (token) {
...
function setupEventListeners() {
    ...
    if (closeFeeBtn) {
        closeFeeBtn.addEventListener('click', closeFeeModal);
    }
    ...
    if (saveFeeBtn) {
        saveFeeBtn.addEventListener('click', saveFeeBalance);
    }


    const closeAssignBtn = document.querySelector('.close-assign-modal');
    if (closeAssignBtn) {
    ...
    if (saveAssignBtn) {
        saveAssignBtn.addEventListener('click', saveAssignStaff);
    }
...
async function loadDataFromBackend() {
    try {
        ...
        adminData.students = adminData.students.map(s => ({
            ...s,
            class: 'N/A',
            feeBalance: s.studentData?.feeBalance ?? 0,
        ...
        try {
            const pendingResponse = await apiFetch(`${API_BASE}/api/users/pending`, {
                headers: {
                    'Authorization': `Bearer ${adminData.authToken}`,
                    'Content-Type': 'application/json'
                }
...
function renderStudentsTable(students) {
    ...
    if (students.length === 0) {
        tbody.innerHTML = '<tr class="empty-row"><td colspan="9" class="empty-state">No students found</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = students.map(student => `
        <tr>
            ...
            <td>${formatFee(student.feeBalance)}</td>
            ...
            <td>
                <div class="table-actions">
                    ...
                    <button class="btn-fee" onclick="openFeeModal(${student.id})">Set Fee</button>
                    <button class="btn-assign" onclick="openAssignStaffModal(${student.id})">Assign Staff</button>
...
function renderStaffTable(staff) {
    ...
    if (staff.length === 0) {
        tbody.innerHTML = '<tr class="empty-row"><td colspan="8" class="empty-state">No staff members found</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = staff.map(member => `
        <tr>
            ...
            <td>${formatSubjects(member.subjects || [])}</td>
...
function normalizeSubjects(value) {
    ...
    const allowed = new Set(SUBJECT_OPTIONS.map(opt => opt.value));
    return list
...
function formatSubjects(list) {
    ...
    const labelMap = SUBJECT_OPTIONS.reduce((acc, opt) => {
...
function renderApprovalsTable(pendingUsers) {
    ...
    if (!pendingUsers || pendingUsers.length === 0) {
        tbody.innerHTML = '<tr class="empty-row"><td colspan="9" class="empty-state">No pending approvals</td></tr>';
        return;
    }
    
    tbody.innerHTML = pendingUsers.map(user => {
        const requestedRole = user.requested_role || user.role || 'student';
        const requestedSubjects = normalizeSubjects(user.requested_subjects || user.subjects);
        const requestedSubjectsLabel = formatSubjects(requestedSubjects);
        const subjectChoices = SUBJECT_OPTIONS.map(opt => {
            const checked = requestedSubjects.includes(opt.value) ? 'checked' : '';
            return `
                <label class="subject-choice">
                    <input type="checkbox" data-subject-for="${user.id}" value="${opt.value}" ${checked}>
                    <span>${opt.label}</span>
                </label>
            `;
        }).join('');
        ...
        return `
            <tr>
                ...
                <td>${user.email}</td>
                <td>${requestedRole}</td>
                <td class="subject-list">${requestedSubjectsLabel}</td>
                <td><div class="subject-choices">${subjectChoices}</div></td>
                <td>
...
async function loadStaffActivities() {
    try {
        const response = await apiFetch(`${API_BASE}/api/reports/staff-activities`, {
            headers: {
                'Authorization': `Bearer ${adminData.authToken}`,
                'Content-Type': 'application/json'
            }
...
function openFeeModal(studentId) {
    ...
    adminData.feeStudentId = studentId;
    const nameEl = document.getElementById('feeStudentName');
    if (nameEl) {
        nameEl.textContent = `${student.name} (${student.email})`;
    }
    const input = document.getElementById('feeBalanceInput');
    if (input) {
        const feeValue = student.feeBalance ?? 0;
        input.value = Number(feeValue);
    }
...
function closeFeeModal() {
    ...
    adminData.feeStudentId = null;
}

async function saveFeeBalance() {
    if (!adminData.feeStudentId) {
    ...
    const feeBalance = Number(value);
    if (Number.isNaN(feeBalance) || feeBalance < 0) {
        showNotification('Enter a valid fee balance', 'error');
        return;
    }

    try {
        const response = await apiFetch(`${API_BASE}/api/users/${adminData.feeStudentId}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${adminData.authToken}`,
                'Content-Type': 'application/json'
            ...
            body: JSON.stringify({
                studentData: { feeBalance: feeBalance },
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.error || 'Failed to update fee balance');
        }

        const student = adminData.students.find(s => s.id === adminData.feeStudentId);
        if (student) {
            student.feeBalance = feeBalance;
            student.studentData = student.studentData || {};
            student.studentData.feeBalance = feeBalance;
        }
...
async function saveAssignStaff() {
    ...
    rows.forEach(row => {
        ...
        requests.push(apiFetch(`${API_BASE}/api/classes/${classId}/enroll`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${adminData.authToken}`,
                'Content-Type': 'application/json'
...
async function handleFormSubmit(e) {
    ...
    if (mode === 'edit') {
        ...
        try {
            const response = await apiFetch(`${API_BASE}/api/users/${recordId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${adminData.authToken}`,
                    'Content-Type': 'application/json'
...
function handleApprovalAction(e) {
    ...
    const subjectInputs = document.querySelectorAll(`input[data-subject-for="${userId}"]:checked`);
    const subjects = Array.from(subjectInputs).map(input => input.value);
    
    approveUser(userId, role, subjects);
}

// Approve a pending user
async function approveUser(userId, role, subjects) {
    try {
        const response = await apiFetch(`${API_BASE}/api/users/${userId}/approve`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${adminData.authToken}`,
                'Content-Type': 'application/json'
...
async function loadAuditLogs() {
    ...
    try {
        const response = await apiFetch(`${API_BASE}/api/audit?limit=100`, {
            headers: {
                'Authorization': `Bearer ${adminData.authToken}`,
                'Content-Type': 'application/json'
            }
...
async function restoreBackup() {
    ...
    try {
        const response = await apiFetch(`${API_BASE}/api/backup/restore`, {
            method: 'POST',
            body: formData
        });
